...
#    foreach img $svgim {}
    foreach img $tekans {
#puts "-> $img"
	if {[catch {svg2can::SVGFileToCanvas $t.b $img } gr] } {
	    puts "Bad file: $img er=$gr"
	} else {
#Размещение со сдаигом в низ 
	    if {1} {		
#ОБрамляющий прямоугольник - белый
		foreach {x1 y1 x2 y2} [.test.c bbox $gr] {break}
		if {$zz != ""} {
		    $zz destroy
		    set zz ""
		}
#		set zz [ibutton new $t.c1 -x 5m -y 2.5c -width [expr {$x2 - $x1 + 2 * $m1}] -height [expr {$y2 - $y1 + 2 * $m1}] -text "" -help "$img" -pad 1m]
		set zz [ibutton new $t.c1 -x 5m -y 2.5c -width [expr {457 + 2 * $m1}] -height [expr {356 + 2 * $m1}] -text "" -help "$img" -pad 1m]
		pack $t.c1 -in $t.c -side left  -padx 5m -pady 2.5c
		$zz config -isvg "$t.b $gr"
		
		$t.b delete $gr
		puts "Ok file: $img ibutton=$zz"
	    }
#Подключить bind
	}
    }
