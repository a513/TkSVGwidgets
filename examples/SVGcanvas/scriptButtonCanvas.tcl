...
#Menu
    variable t
    if {[$::butm1 config -menu] !=""} {
	[$::butm1 config -menu] destroy
	$::butm1 config -menu {}
    }
    variable dtype
    set menu [cmenu new $t.c -direction $dtype]
    $menu add check -text check1 -variable z2
    $menu add command -text [mc Command] -command {puts "Enter Button Command"}
...
#####################
wm geometry $t 800x550+50+50
wm minsize $t 700 500
wm title $t "SVG-widgets: all to one canvas with resize"
...
set xgrad [cbutton new $t.c -type round -x 2m -y 2m  -text [mc "Update gradient"] -command "tk busy hold .test;variable tkpfr;updategrad \[\$tkpfr canvas] \[\$tkpfr config -fillnormal];tk busy forget .test" -width 150 -height 25]
#$xgrad place -in $t.c -x 2m -y 2m
#


set ::butm1 [cbutton new $t.c -x 320 -y 260 -text viewMenu -menu {}]

set yrm 300
foreach rmbut "up down left right" {
    set rc1 [cbutton new $t.c -type radio  -text $rmbut -variable dtype -value $rmbut -x 330 -y $yrm -command {createMenu}]
    incr yrm 30
}
...
bind [$went entry] <ButtonRelease-3> "puts \"Selected svg-widget: $went\""
set tg [mc "Are you sure you\nwant to quit?"]
wm protocol $t WM_DELETE_WINDOW "exitcanvas [set t]  \"[set tg]\""
